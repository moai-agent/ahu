...
- Add release-branch triggers so the candidate receives the same checks as
  `main`.
- Gate live integrations explicitly. A macOS cmux probe may run only when the
  runner provides a reachable cmux instance; otherwise emit a visible notice
  and skip it. Never turn a missing optional integration into a false pass.
...
configuration. Keep issue details and release findings in a verified private
